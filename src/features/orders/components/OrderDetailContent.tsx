...
import { ORDER_STATUS_FUN } from "../constants";
...
import { ORDER_STATUS, ORDER_STATUS_LABEL, type OrderStatus } from "@/constants/status.constants";
...
export interface OrderDetailContentProps {
  orderId: string;
  /** Fires once the order loads/refreshes — lets the host render its own title. */
  onOrderLoaded?: (order: Order) => void;
}

/**
 * The order detail body: status hero, progress timeline, itemized bill with
 * product/add-on photos. Shared by the pushed full-screen route (phone) and
 * the right-docked sidebar (tablet/laptop) so both stay in lockstep.
 */
export function OrderDetailContent({ orderId, onOrderLoaded }: OrderDetailContentProps) {
  const queryClient = useQueryClient();

  const query = useQuery({
    queryKey: QK.order(orderId),
    queryFn: () => customerService.myOrder(orderId),
    enabled: !!orderId,
  });

  useEffect(() => {
    if (query.data) onOrderLoaded?.(query.data);
    // onOrderLoaded is expected to be stable (or the host memoizes it) — only
    // re-fire when the order data itself changes.
  ...
  const progress = cancelled ? 0 : pipeline.length > 1 ? reachedIndex / (pipeline.length - 1) : 1;
    ...
    <div className="space-y-4">
      ...
      <div
        className={cn(
          ...
          cancelled ? "bg-danger/10" : "bg-gradient-to-b from-primary/12 to-surface",
        )}
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="mx-auto mt-1 max-w-xs text-[13px] font-medium text-muted-foreground">
              {cancelled && order.cancellationReason ? order.cancellationReason : fun?.description}
            </p>
          </motion.div>
        </AnimatePresence>
      ...
      {/* Items + bill — each line shows the product photo, add-ons show theirs too */}
      <div className="rounded-3xl bg-surface p-5 shadow-card">
                  ...
                  <div className="mt-2 space-y-1.5">
                    {item.addons.map((addon, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <ItemThumb src={addon.imageUrl} alt={addon.name} size="sm" />
                        <p className="text-[11px] font-medium text-primary">+ {addon.name}</p>
                      </div>
                    ))}
                  </div>
                )}
...
/** Small rounded product/add-on photo tile; falls back to a soft placeholder. */
function ItemThumb({
  src,
  alt,
  size = "md",
  ...
  const dims = size === "sm" ? "h-7 w-7 rounded-lg" : "h-12 w-12 rounded-2xl";
  if (!src) {
    return (
      <span
        className={cn(
        ...
        aria-hidden
      >
        <ReceiptArt className={size === "sm" ? "h-4 w-4" : "h-6 w-6"} />
      </span>
    );
  }
  return (
    <img
      ...
      alt={alt}
      loading="lazy"
      className={cn("shrink-0 bg-surface-2 object-cover", dims)}
    />
  );
}
